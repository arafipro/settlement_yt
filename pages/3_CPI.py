...
if(save and selected_last_month.month < 10):
	# 空のファイルのパスを指定
	empty_txt_file = pathlib.Path(f"cpi_{selected_last_month.year}0{selected_last_month.month}.txt")
	# 空のファイルを作成
	...
	# 空のファイルのパスを指定
	empty_txt_file = pathlib.Path(f"cpi_{selected_last_month.year}{selected_last_month.month}.txt")
	# 空のファイルを作成
	empty_txt_file.touch()
	# 空のファイルに読み込んだtxtを書き込む
	empty_txt_file.write_text(str_title + str)
